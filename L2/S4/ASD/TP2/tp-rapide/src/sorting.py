...
def partition (s, cmp):
    ...
    while i < len(s['data']):
        ...
        elif cmp(pivot,s['data'][i]) == -1:
            temp = s['data'][i]

            if cmp(pivot, s['data'][j]) == -1 and j>p:

                while cmp(pivot,s['data'][j])==-1 and j>p:
                    j=j-1

                if j>p:
                    s['data'][p] = s['data'][j]
                    s['data'][j] = temp
                    s['data'][i] = random_pivot
                    p = i
                    i = i + 1
                    j = j - 1

                else:
                    return ( { 'left' : 0 , 'right' : p - 1 , 'data' : np.array(s['data'][0:p])} , {'left': p + 1 , 'right' : len(s['data']) - 1 , 'data': np.array(s['data'][p+1:])})

def random_pivot(s):
    """
    Return random index of tabl's slice
    ...
    :param s: A slice of is a dictionary with 3 fields :
              ...
              - right: right bound of the slice.
    :type s: dict
    :return: a random index of slice
    :rtype: int
    """
    return randint(s['left'], s['right'])
